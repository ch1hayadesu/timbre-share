...
角色C：等等我！我也要去！</pre>
      ...
      <BaseButton type="primary" size="lg" :disabled="!hasScriptFile" @click="startDub">开始智能配音</BaseButton>
    </Card>
    ...
    <Card v-if="phase === 'result'">
      <div class="result-header">
        <h3>✓ 配音生成完成</h3>
        <BaseTag type="success">3个角色 · 15句台词</BaseTag>
      </div>

      <AudioPlayer title="🎭 完整配音音频" :current-time="'0:00'" :duration="'2:35'" :progress="0"
        info="MP3 · 128kbps · 2:35">
        <template #actions>
      ...
      <table class="role-table">
        <thead>
          <tr><th>角色</th><th>匹配音色</th><th>台词数</th><th>主要情感</th><th>操作</th></tr>
        </thead>
        <tbody>
          <tr v-for="role in roles" :key="role.name">
            ...
            <td><BaseButton type="text" size="sm" @click="showToast('info','音色更换功能开发中')">更换音色</BaseButton></td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="result-actions">
        ...
        <BaseButton type="primary" @click="showToast('success','重新生成已提交')">修改后重新生成</BaseButton>
      </div>
    </Card>
  </div>
</template>
...
const roles = [
  {
    name: '角色A', voice: '温柔女声', lines: 5,
    emotions: [{ type: 'joy', label: '喜悦' }, { type: 'neutral', label: '中性' }]
  },
  {
    name: '角色B', voice: '沉稳男声', lines: 5,
    emotions: [{ type: 'neutral', label: '中性' }, { type: 'joy', label: '喜悦' }]
  },
  {
    name: '角色C', voice: '活泼少女', lines: 5,
    emotions: [{ type: 'joy', label: '喜悦' }, { type: 'fear', label: '恐惧' }]
  },
]
...
function startDub() {
  ...
  const phases = [
    { step: 0, text: 'AI 正在分割角色...', prog: 25 },
    { step: 1, text: '正在分析台词情感...', prog: 50 },
    { step: 2, text: '正在匹配合适音色...', prog: 75 },
    { step: 3, text: '正在生成配音音频...', prog: 100 },
  ]
  ...
  const interval = setInterval(() => {
    if (idx < phases.length) {
      const p = phases[idx]
      scriptStatusText.value = p.text
      scriptProgress.value = p.prog
      scriptCurrent.value = p.step
      idx++
    } else {
