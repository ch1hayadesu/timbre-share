...
CREATE TABLE "user" (
    ...
    membership_level SMALLINT        DEFAULT 0,
...
COMMENT ON COLUMN voice.status IS '0:处理中 1:就绪 2:失败（已移除3:已分享，由voice_share管理）';
...
COMMENT ON COLUMN voice_download.share_id IS '分享ID（分享者删除后可为NULL，已下载用户不受影响）';
...
| voice | idx_voice_name_trgm | GIN trgm | 音色名称模糊搜索 | REQ-033 |
