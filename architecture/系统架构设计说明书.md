...
│                  Nginx反向代理 + JWT鉴权中间件                        │
├──────────────────────────────────────────────────────────────────┤
│                   业务后端 (Business Backend — Java Spring Boot)      │
│    ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐          │
│    │ 用户模块  │ │ 音色管理  │ │ TTS记录  │ │ 剧本配音  │          │
│    └──────────┘ └──────────┘ └──────────┘ └──────────┘          │
│    ┌──────────┐ ┌──────────┐ ┌──────────────────┐               │
│    │ 分享平台  │ │ 通知服务  │ │ AI后端代理(HTTP) │               │
│    └──────────┘ └──────────┘ └──────────────────┘               │
├──────────────────────────────────────────────────────────────────┤
│              AI后端 (AI Backend — Python FastAPI)                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │               AI推理抽象层 (Inference Abstraction)          │   │
│  │  统一接口：clone() / synthesize() / analyze_emotion()      │   │
 │  ├─────────────────┬─────────────────┬────────────────────┤   │
 │  │ GPT-SoVITS v2  │  CosyVoice 2/3  │  hubert-base-ch-SER │   │
 │  │ (深度克隆 GPU)  │ (零样本/流式GPU)│  (语音情感分析 CPU) │   │
 │  ├─────────────────┴─────────────────┴────────────────────┤   │
 │  │                    MockEngine                          │   │
 │  │                    (开发测试)                           │   │
 │  └────────────────────────────────────────────────────────┘   │
 │  ┌─────────────────┐ ┌──────────────────┐ ┌──────────────┐   │
 │  │ 剧本解析引擎     │ │ 音频处理服务      │ │ Celery任务    │   │
 │  │ (NLP+正则)      │ │ (FFmpeg)         │ │  优先级队列    │   │
 │  └─────────────────┘ └──────────────────┘ └──────────────┘   │
├──────────────────────────────────────────────────────────────────┤
│                        数据层 (Data)                               │
│    ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐        │
│    │  MySQL 8 │ │  Redis   │ │ MinIO/OSS│ │ 本地缓存  │        │
│    │ 业务数据  │ │ 缓存/队列 │ │ 音频/模型 │ │          │        │
│    └──────────┘ └──────────┘ └──────────┘ └──────────┘        │
└──────────────────────────────────────────────────────────────────┘

▲ 业务后端(Java)与AI后端(Python)通过内部HTTP REST API通信
▲ 前端请求 → Nginx → 业务后端 → AI后端(按需)
```
...
```python
ENGINE_REGISTRY = {
    "gpt_sovits": GPTSoVITSEngine,      # GPU 深度克隆主推
    "cosy_voice": CosyVoiceEngine,      # GPU 零样本/流式主推
...
def get_engine(task_type: str) -> InferenceEngine:
    ...
    elif task_type == "instant_clone" or task_type == "tts":
        return ENGINE_REGISTRY["cosy_voice"]()
    return ENGINE_REGISTRY["cosy_voice"]()

| 任务类型 | 首选引擎 | 备选引擎 |
|----------|----------|----------|
| 深度克隆 | GPT-SoVITS v2 GPU | CosyVoice 2/3 GPU |
| 即时克隆/零样本 | CosyVoice 2/3 GPU | IndexTTS-2 GPU |
| TTS合成 | CosyVoice 2/3 GPU | GPT-SoVITS v2 GPU |
| 语音情感分析 | hubert-base-ch-SER CPU | SenseVoice GPU |
...
| 分层 | 技术 | 版本 | 说明 |
|------|------|:----:|------|
| 前端 | Vue 3 / React + TypeScript + Vite | 3.x / 18.x | 响应式SPA |
| 业务后端 | Java + Spring Boot 3 + Spring Data JPA | 21 / 3.3.x | 核心业务逻辑 |
| AI后端 | Python + FastAPI + Celery | 3.12 / 0.110.x | AI推理独立服务 |
| 数据库 | MySQL | 8.0 | 主业务数据库 |
| 缓存/队列 | Redis | 7 | 缓存 + 队列Broker |
| 对象存储 | MinIO | latest | 音频/模型文件 |
| AI深度克隆 | GPT-SoVITS | v2 | 少样本微调，5语种(GPU) |
| AI零样本/流式 | CosyVoice | 2.0/3.0 | 零样本克隆+流式150ms(GPU) |
| 语音情感识别 | hubert-base-ch-SER | v1 | 6类语音情感，准确率97.2%(CPU) |
| 情感分析(备选) | SenseVoice | latest | 阿里多模态语音理解(GPU) |
| 异步任务 | Celery | 5.3 | AI端优先级任务队列 |
| 音频处理 | FFmpeg | 6.x | 格式转换/拼接 |
| 部署编排 | Docker Compose | 3.8 | 双服务容器编排 |
| 监控 | Prometheus + Grafana | latest | 双服务维度监控 |

> **架构说明**：业务后端(Java)与AI后端(Python)通过内部REST API通信。
> 业务后端处理用户管理/音色CRUD/TTS记录/分享等纯业务逻辑，
> AI后端处理语音克隆/TTS推理/情感分析/剧本解析等计算密集型任务。
